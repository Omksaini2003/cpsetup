...
void adjust(string& l, string& r){
    while(l.size()<r.size()){
        l = '0'+l;
    }
    if(l.size()>r.size()) adjust(r,l);
}

void solve(){
    int L,R; cin>>L>>R;
    string l= decimaltobinary(L), r= decimaltobinary(R);
    ...
    int A = binarytodecimal(a), B = binarytodecimal(b), C;
    // if(A<=R && B<=R && A>=L && B>=L) cout<<"true\n";
    for(int i=L; i<=R; i++){
        C=i;
        if(C!=A && C!=B) break;
    }
